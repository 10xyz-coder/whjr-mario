...
function game(){
  ...
  if(gameConfig.status==='start'){
    ...
    textSize(40);
    textAlign(CENTER);
    text("Move Head to Start and Play ", gameConfig.screenX/2, gameConfig.screenY/2);
    textSize(40);
...
function manualControl(character){
  ...
  if(character.live){
    ...
    if(!(nose.x < 200)&&!(nose.x > 400)&&!(nose.y < 100)){ 
...
function jumping(character){
	if( ((nose.y < 150)&&character.live) || (touchIsDown&&character.live) ){
...
function scores(character){
  ...
  gameConfig.scores=character.coins*10+character.kills*25+gameConfig.timeScores;
